...
const form = document.querySelector(".lorem-form"),
  ...
  output = document.querySelector(".lorem-text");

form.addEventListener("submit", (e) => {
  e.preventDefault();
  const input = parseInt(amount.value),
    randomNum = Math.floor(Math.random() * text.length);
  if (isNaN(input) || input < 1 || input > 9) {
    output.innerHTML = `<p class='output'>${text[randomNum]}</p>`;
  } else {
    let outputText = text.slice(0, input);
    outputText = outputText
      .map((p) => {
        return `<p class='output'>${p}</p>`;
      })
      .join("");
